...
try:
    lan_f = open("C:\\yzkrz\\Draw\\lan", "r", encoding='utf-8')
    ...
    if selected_language is None:
        sys.exit()
try:
    f16 = open("C:\\yzkrz\\Draw\\lan", "w", encoding='utf-8')
except FileNotFoundError:
    os.mkdir("C:\\yzkrz\\Draw\\")
    f16 = open("C:\\yzkrz\\Draw\\lan", "w", encoding='utf-8')
...
def paint_text_first(text: str, color: tuple[int, int, int], bg_color: tuple[int, int, int] or None, x: int, y: int,
                     mod: str, aa: bool, size: int = 32):
...
loading_lan = ["正在加载...", "Loading...", "Chargement...", "Cargando...", "Загрузка...", "ロード中..."]


screen = pygame.display.set_mode((400, 400), pygame.NOFRAME)
pygame.display.set_caption(" ")
screen.fill((230, 230, 230))
paint_text_first(loading_lan[e], (0, 0, 0), None, 200, 200, "center", True, 32)
pygame.display.update()

pid_lan = ["本软件正在其他地方运行，请将其关闭，然后重试。",
           "This software is running elsewhere. Please close it and try again.",
           "Ce logiciel fonctionne ailleurs, veuillez l'éteindre et réessayer.",
           "Este software se está ejecutando en otro lugar. Por favor, Ciérrelo e inténtelo de nuevo.",
           "Это программное обеспечение работает в другом месте, пожалуйста, отключите его, а затем попробуйте снова.",
           "このソフトウェアは他の場所で実行中です。オフにして再試行してください。"]
...
for pid in pids:
    try:
        ...
        if pid_name == "py.exe":
            ...
            print(cmd_line)
            try:
                if cmd_line[1].count("Draw_main.py") > 0:
                    pid_i += 1
            except IndexError:
                pass
        elif pid_name == "Draw_main.exe":
            pid_i += 1
    except psutil.NoSuchProcess:
        pass
print(pid_i)
if pid_i > 1:
    easygui.msgbox(pid_lan[e])
    exit()
...
class Key:
    def __init__(self):
        global key_lan
        try:
            ask1 = open("C:\\yzkrz\\Draw\\asked.dll", "r")
            ask1.close()
        except FileNotFoundError:
            try:
                os.remove("C:\\yzkrz\\Draw\\key")
            except FileNotFoundError:
                pass
            try:
                os.remove("C:\\yzkrz\\Draw\\used.dll")
            except FileNotFoundError:
                pass
            try:
                os.mkdir("C:\\yzkrz\\Draw\\")
                ...
                pass
            asked = open("C:\\yzkrz\\Draw\\asked.dll", "w")
            asked.write("")
            asked.close()
        ...
        try:
            self.key_file = open("C:\\yzkrz\\Draw\\key", "r")
            ...
            self.key = ""
            for _ in range(8):
                self.key += str(random.randint(1, 9))
            self.key_file = open("C:\\yzkrz\\Draw\\key", "w")
...
try:
    ask = open("C:\\yzkrz\\Draw\\asked.dll", "r")
    ask.close()
except FileNotFoundError:
    try:
        os.remove("C:\\yzkrz\\Draw\\key")
...
# noinspection PyBroadException
try:
    used_file = open(r"C:\\yzkrz\\Draw\\used.dll", "r")
    used = used_file.read()
    used_file.close()
...
if days <= 0:
    ...
    u = open("C:\\yzkrz\\Draw\\used.dll", "w")
    u.write(str(_T2))
    u.close()
...
try:
    d_f = open("C:\\yzkrz\\Draw\\data", "r")
    df = eval(d_f.read())
    d_f.close()
except FileNotFoundError:
    df = {"1": 1, "2": 1, "3": 1, "4": 1, "5": 1}
    d_f2 = open("C:\\yzkrz\\Draw\\data", "w")
    d_f2.write(str(df))
    d_f2.close()
...
def exe_exit():
    ...
    f1 = open("C:\\yzkrz\\Draw\\last_data", "w", encoding='utf-8')
    f1.write(str(name))
    f1.close()
    f2 = open("C:\\yzkrz\\Draw\\last", "w", encoding='utf-8')
    f2.write(str(last))
    f2.close()
    f3 = open("C:\\yzkrz\\Draw\\l_list", "w", encoding='utf-8')
    for f3_i in l_list:
        f3.write(str(f3_i) + "\n")
    f3.close()
    f17 = open("C:\\yzkrz\\Draw\\lan", "w", encoding='utf-8')
    selected_language = f17.write(str(selected_language))
    f17.close()
    f19 = open("C:\\yzkrz\\Draw\\useLast", "w", encoding='utf-8')
            ...
            if setting_open:
                if setting_x <= 0:
                    ...
                    if xy_x <= x <= 400 and 360 <= y <= 400:
                        ...
                        os.system('start xy/' + str(xyz) + '.html')
    ...
    if inputting != "":
        if int(inputting) < 1000:
            if not fcl_save:
                ...
                ul = open("C:\\yzkrz\\Draw\\useLast", "w")
...
total = 0
name_range = 0
f8 = open("C:\\yzkrz\\Draw\\data", "r", encoding='utf-8')
name = eval(f8.read())
f8.close()
lim = []
try:
    f9 = open("C:\\yzkrz\\Draw\\last", "r", encoding='utf-8')
    last = int(f9.read())
    f9.close()
except FileNotFoundError:
    last = 0
...
try:
    f10 = open("C:\\yzkrz\\Draw\\useLast", "r", encoding='utf-8')
...
if not (last >= len(name) * useLast):
    # noinspection PyBroadException
    try:
        f11 = open("C:\\yzkrz\\Draw\\last_data", "r", encoding='utf-8')
        name = eval(f11.read())
        f11.close()
    except:
        f12 = open("C:\\yzkrz\\Draw\\data", "r", encoding='utf-8')
        name = eval(f12.read())
        f12.close()
else:
    last = 0
...
try:
    f13 = open("C:\\yzkrz\\Draw\\l_list", "r", encoding='utf-8')
    ...
    for event in lim:
        ...
        if isin(event[0], event[1], r):
            ...
            if last >= len(name) * useLast:
                f14 = open("C:\\yzkrz\\Draw\\data", "r", encoding='utf-8')
                name = eval(f14.read())
                f14.close()
                last = 0
            break
...
check_lan = ["正在检查更新。", "Checking for updates.", "Vérification des mises à jour.", "Comprobando actualizaciones.",
             "проверка обновления.", "更新をチェックしています。"]
screen.fill((230, 230, 230))
paint_text_first(check_lan[e], (0, 0, 0), None, 200, 200, "center", True, 26)
pygame.display.update()
...
while True:
    ...
    for i in range(len(name)):
        ...
        lim.append([txt, txt + name[str(1 + i)] - 1])
    if last >= len(name) * useLast:
        f15 = open("C:\\yzkrz\\Draw\\data", "r", encoding='utf-8')
        name = eval(f15.read())
        f15.close()
        last = 0
    ...
    if state == "randing":
        ...
        paint_text(installed_language[selected_language][2], (255, 255, 255), (100, 150, 255), 200, 336, "center", True,
                   32)
        if new_version:
            screen.blit(update_img, (365, 365))
    elif state == "result":
        screen.blit(settings, (355, 0))
        pygame.draw.rect(screen, (100, 150, 255), (75, 300, 250, 75), 0, border_radius=20)
        pygame.draw.rect(screen, (125, 125, 125), (50, 40, 300, 230), 2, border_radius=20)
